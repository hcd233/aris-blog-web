  ...
  <div class="max-w-4xl mx-auto py-8 px-4">
    ...
    <template v-else>
      <div class="flex justify-between items-center mb-8">
        <h1 class="text-3xl font-bold text-primary-color">编辑文章</h1>
...
const fetchPost = async () => {
  ...
  try {
    ...
    if (store.currentPost) {
      form.value = {
        title: store.currentPost.title,
        content: store.currentPost.content || '',
        categoryId: store.currentPost.category?.id.toString() || '',
        tags: store.currentPost.tags.map(tag => tag.slug),
        status: store.currentPost.status
      }
    }
  } catch (err: any) {
...
const handleSave = async () => {
  ...
  try {
    ...
    // 1. 更新文章基本信息
    await store.updatePost(userName as string, slug as string, {
      title: form.value.title,
      categoryId: form.value.categoryId ? parseInt(form.value.categoryId) : undefined
    })

    // 2. 如果内容有变化，创建新版本
    if (store.currentVersion?.content !== form.value.content) {
      await store.createArticleVersion(userName as string, slug as string, form.value.content)
    }
