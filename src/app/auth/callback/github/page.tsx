...
function GitHubCallback() {
  ...
  useEffect(() => {
    if (code && state) {
      authService.handleGitHubOAuthCallback(code, state)
        .then((data) => {
          // DEBUG LOG: Log data received from backend
          console.log('[CallbackPage] Data received from handleGitHubOAuthCallback:', data);
          
          if (data.accessToken) {
            localStorage.setItem('accessToken', data.accessToken);
            // DEBUG LOG: Log token being set
            console.log('[CallbackPage] accessToken set in localStorage:', data.accessToken);
          } else {
            console.warn('[CallbackPage] No accessToken received in data:', data);
          }

          if (data.refreshToken) {
            localStorage.setItem('refreshToken', data.refreshToken);
            console.log('[CallbackPage] refreshToken set in localStorage:', data.refreshToken);
          } else {
            console.warn('[CallbackPage] No refreshToken received in data:', data);
          }

          toast.success("Login Successful", { // 使用 sonner 的 success 方法
        ...
        .catch((error) => {
          ...
          toast.error("Login Failed", { // 使用 sonner 的 error 方法
            description: error.message || 'An error occurred during GitHub OAuth callback.',
          });
